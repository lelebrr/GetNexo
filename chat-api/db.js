...
const initSchema = () => {
  ...
  if (usersCount === 0) {
    ...
    // Admin
    const adminPass = process.env.ADMIN_PASSWORD || require('crypto').randomBytes(16).toString('hex');
    if (!process.env.ADMIN_PASSWORD) {
      console.log('WARNING: ADMIN_PASSWORD not set. Generated temporary password for admin:', adminPass);
    }
