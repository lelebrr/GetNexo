...
const initSchema = () => {
  ...
  const indexes = [
    ...
    `CREATE INDEX IF NOT EXISTS idx_tickets_last_message ON tickets(last_message_at DESC, created_at DESC)`
  ];
  ...
  indexes.forEach(sql => {
    try {
      ...
      // Ignore if index already exists or column missing (during migration)
    }
  });
