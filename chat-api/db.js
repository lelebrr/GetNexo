...
const initSchema = () => {
  ...
  // Impact: reducing queries from O(N) full table scans to O(log N) index lookups
  try {
    ...
    db.prepare('CREATE INDEX IF NOT EXISTS idx_transactions_contact_status ON transactions(contact_id, status)').run();
