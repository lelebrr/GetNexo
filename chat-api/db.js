...
const initSchema = () => {
  ...
  if (usersCount === 0) {
    ...
    // Admin
    const adminPass = process.env.ADMIN_DEFAULT_PASSWORD || require('crypto').randomBytes(16).toString('hex');
    ...
    const masterPass = process.env.MASTER_DEFAULT_PASSWORD || require('crypto').randomBytes(16).toString('hex');
    insertUser.run(4, 'lelebrr@gmail.com', bcrypt.hashSync(masterPass, 10), 'Lele', 'superadmin', 1, null);
