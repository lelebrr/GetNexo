...
router.get('/stats', (req, res) => {
    try {
        // ⚡ Bolt: Combined sequential COUNT(*) queries into a single query using conditional aggregation, reducing DB roundtrips from 3 to 1 per table.
        ...
        const peerCount = peerStats.total || 0;
        const peersTrusted = peerStats.trusted || 0;
        ...
        const messageStats = db.prepare(`
            ...
            FROM a2a_messages
        `).get() || { total: 0, today: 0 };
        const messageCount = messageStats.total || 0;
        const messagesToday = messageStats.today || 0;
