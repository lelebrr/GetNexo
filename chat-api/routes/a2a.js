...
router.get('/stats', (req, res) => {
    try {
        ...
        // Optimization: Single query to fetch total tasks and count by status
        // to prevent multiple full-table scans
        const tasksStatsRow = db.prepare(`
            SELECT
                COUNT(*) as total,
            ...
            FROM a2a_tasks
        `).get();
        const taskCount = tasksStatsRow?.total || 0;
        const tasksCompleted = tasksStatsRow?.completed || 0;
        const tasksPending = tasksStatsRow?.pending || 0;
