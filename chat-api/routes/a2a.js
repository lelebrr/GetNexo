...
router.get('/stats', (req, res) => {
    try {
        ...
        // Combine sequential COUNT(*) queries into single queries using conditional aggregation
        // Impact: Reduces database latency and table scans from 7 queries to 4 queries.
        const messageStats = db.prepare("SELECT COUNT(*) as cnt, SUM(CASE WHEN DATE(created_at) = DATE('now') THEN 1 ELSE 0 END) as today FROM a2a_messages").get() || { cnt: 0, today: 0 };
        const taskStats = db.prepare("SELECT COUNT(*) as cnt, SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END) as completed, SUM(CASE WHEN status = 'pending' THEN 1 ELSE 0 END) as pending FROM a2a_tasks").get() || { cnt: 0, completed: 0, pending: 0 };
        const peerStats = db.prepare("SELECT COUNT(*) as cnt, SUM(CASE WHEN trusted = 1 THEN 1 ELSE 0 END) as trusted FROM a2a_peers").get() || { cnt: 0, trusted: 0 };
        ...
        res.json({
            messages: {
                total: messageStats.cnt || 0,
                today: messageStats.today || 0
            },
            tasks: {
                total: taskStats.cnt || 0,
                completed: taskStats.completed || 0,
                pending: taskStats.pending || 0
            },
            peers: {
                total: peerStats.cnt || 0,
                trusted: peerStats.trusted || 0
            },
