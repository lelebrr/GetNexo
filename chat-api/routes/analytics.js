...
router.get('/dashboard-stats', (req, res) => {
    try {
        // Receita Total (24h) e Total de Vendas (24h) combinados em uma única query
        // ⚡ Bolt: Combines SUM and COUNT into a single query for transactions in the last 24h
        // Impact: Reduces queries from 2 to 1, cutting DB roundtrips and table scans by 50% on this endpoint.
        const salesData24h = db.prepare(`
            SELECT SUM(amount) as total, COUNT(*) as count
            FROM transactions 
            WHERE created_at >= datetime('now', '-1 day') AND status = 'paid'
        `).get() || { total: 0, count: 0 };

        const revenue24h = { total: salesData24h.total || 0 };
        const salesCount24h = { count: salesData24h.count || 0 };
