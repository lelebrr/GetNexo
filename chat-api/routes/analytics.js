...
router.get('/dashboard-stats', (req, res) => {
    try {
        // Receita Total e Vendas (24h) combinados - reduzindo 2 consultas para 1
        const txStats24h = db.prepare(`
            SELECT SUM(amount) as total, COUNT(*) as count
            FROM transactions 
            WHERE created_at >= datetime('now', '-1 day') AND status = 'paid'
        `).get() || { total: 0, count: 0 };

        const revenue24h = { total: txStats24h.total || 0 };
        const salesCount24h = { count: txStats24h.count || 0 };
...
router.get('/prediction', (req, res) => {
    try {
        ...
        // Ticket Médio e Receita Total combinados - reduzindo 2 consultas para 1
        const lifetimeTxStats = db.prepare('SELECT AVG(amount) as avgVal, SUM(amount) as sumVal FROM transactions WHERE status="paid"').get() || { avgVal: 0, sumVal: 0 };
        const avgTicket = lifetimeTxStats.avgVal || 0;
        ...
        const totalRevenue = lifetimeTxStats.sumVal || 0;
