...
router.get('/stats', (req, res) => {
    try {
        ...
        // Impact: Reduces database latency and table scans from 4 queries to 2 queries.
        const mandateStats = db.prepare("SELECT COUNT(*) as cnt, SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END) as active, SUM(CASE WHEN status = 'used' THEN 1 ELSE 0 END) as used FROM ap2_mandates").get() || { cnt: 0, active: 0, used: 0 };
        ...
        const activeMandates = mandateStats.active || 0;
        const usedMandates = mandateStats.used || 0;
