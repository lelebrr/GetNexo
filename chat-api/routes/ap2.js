...
router.get('/stats', (req, res) => {
    try {
        // ⚡ Bolt: Combines 4 sequential ap2_mandates queries into 1 using conditional aggregation
        // Measured impact: Reduces full table scans from 4 to 1, preventing redundant disk I/O on large tables.
        ...
        const mandateCount = mandateStats.cnt || 0;
        const activeMandates = mandateStats.active_cnt || 0;
        const usedMandates = mandateStats.used_cnt || 0;

        // ⚡ Bolt: Combines 3 sequential ap2_transactions queries into 1 using conditional aggregation
        // Measured impact: Eliminates 2 redundant table scans and optimizes volume calculation logic.
        const txStats = db.prepare(`
            SELECT
                COUNT(*) as cnt,
                COALESCE(SUM(CASE WHEN status IN ('captured', 'authorized') THEN amount ELSE 0 END), 0) as total,
                COALESCE(SUM(CASE WHEN status IN ('captured', 'authorized') AND DATE(created_at) = DATE('now') THEN amount ELSE 0 END), 0) as today
            FROM ap2_transactions
        `).get() || { cnt: 0, total: 0, today: 0 };

        const transactionCount = txStats.cnt || 0;
        const totalVolume = txStats.total || 0;
        const todayVolume = txStats.today || 0;
