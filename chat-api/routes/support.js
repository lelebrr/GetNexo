...
router.get('/analytics/dashboard', (req, res) => {
    try {
        // Bolt: Optimized 5 sequential queries into a single query to reduce database scans
...
router.get('/admin/stats', (req, res) => {
    try {
        ...
        // Bolt: Optimized 6 sequential queries into a single query to reduce database scans
        const row = db.prepare(`
            SELECT
                COUNT(*) as total,
                SUM(CASE WHEN status = 'open' THEN 1 ELSE 0 END) as open,
