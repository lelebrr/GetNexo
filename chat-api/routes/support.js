...
router.get('/analytics/dashboard', (req, res) => {
    try {
        // ⚡ Bolt: Use conditional aggregation to replace 5 separate queries with a single query
...
router.get('/admin/stats', (req, res) => {
    try {
        ...
        // ⚡ Bolt: Use conditional aggregation to replace 6 separate queries with a single query
        const row = db.prepare(`
            SELECT
                COUNT(*) as total,
                SUM(CASE WHEN status = 'open' THEN 1 ELSE 0 END) as open,
