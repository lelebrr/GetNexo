...
router.put('/:id', (req, res) => {
    try {
        ...
        // Security: Prevent SQL injection by whitelisting allowed fields
        const allowedFields = ['customer_phone', 'customer_name', 'customer_email', 'channel', 'status', 'priority', 'assigned_agent_id', 'assigned_agent_name', 'human_agent', 'tags', 'metadata', 'sentiment', 'sentiment_score', 'last_message', 'last_message_at'];
...
router.get('/stats/overview', (req, res) => {
    try {
        ...
        // Combined 8 sequential COUNT(*) queries into a single query using conditional aggregation.
        // This significantly reduces database roundtrips and latency when calculating ticket overview stats.
        const result = db.prepare(`
            SELECT
                COUNT(*) as total,
        ...
        const stats = {
            ...
            human: result.human || 0,
            today: result.today || 0,
            closed_today: result.closed_today || 0
        };
...
router.get('/stats/agent/:agent_id', (req, res) => {
    try {
        ...
        // ⚡ Bolt: Performance Improvement
        // Combined 7 sequential COUNT(*) queries into a single query using conditional aggregation.
        // This significantly reduces database roundtrips and latency when calculating agent-specific ticket stats.
        const result = db.prepare(`
            SELECT
                COUNT(*) as total,
                SUM(CASE WHEN status = 'open' THEN 1 ELSE 0 END) as open,
