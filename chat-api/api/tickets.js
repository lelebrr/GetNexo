...
router.get('/stats/overview', (req, res) => {
    try {
        // ⚡ Bolt: Using conditional aggregation to calculate all stats in a single database query.
        // This reduces 8 sequential SELECT COUNT queries down to 1, significantly improving performance.
        const query = `
            SELECT
                COUNT(*) as total,
...
router.get('/stats/agent/:agent_id', (req, res) => {
    try {
        ...
        // ⚡ Bolt: Using conditional aggregation to calculate all agent stats in a single database query.
        // This reduces 7 sequential SELECT COUNT queries down to 1, significantly improving performance.
        const query = `
            SELECT
                COUNT(*) as total,
                SUM(CASE WHEN status = 'open' THEN 1 ELSE 0 END) as open,
        ...
        const row = db.prepare(query).get(agent_id);
