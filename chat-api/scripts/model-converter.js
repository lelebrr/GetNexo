...
async function convertGlbToUsdz(glbPath, usdzPath) {
    return new Promise((resolve, reject) => {
        ...
        // Define npx binary based on platform
        const npxCommand = process.platform === 'win32' ? 'npx.cmd' : 'npx';

        // Example using gltf-to-usdz (npm package) using execFile to avoid command injection
        execFile(npxCommand, ['gltf-to-usdz', glbPath, usdzPath], { shell: process.platform === 'win32' }, (error, stdout, stderr) => {
            if (error) {
