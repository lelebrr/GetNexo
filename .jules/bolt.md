...
## 2025-02-28 - Optimizing Multiple COUNT(*) Queries
**Learning:** In analytical endpoints (like `/dashboard-stats`), executing sequential \`COUNT(*)\` and \`SUM()\` database queries causes unnecessary latency through multiple table scans.
**Action:** Always combine them into a single query using conditional aggregation \`SUM(CASE WHEN [condition] THEN amount ELSE 0 END)\`. Use fallback logic \`|| 0\` in JavaScript.
