...
## 2025-02-17 - Hardcoded JWT Secret
**Vulnerability:** A hardcoded fallback value was provided for `JWT_SECRET` in `chat-api/server.js`, allowing the application to start with insecure defaults if the environment variable was missing.
**Learning:** Providing fallbacks for critical security secrets defeats the purpose of environment variables, as it fails open instead of failing closed.
**Prevention:** Remove default values for secrets. Explicitly check for their existence and exit the process immediately if they are missing.
**Fix Detail:** Removed the fallback string and ensured the application exits with a fatal error if `JWT_SECRET` is not defined. Updated test setup to provide a dummy secret for the test environment.
