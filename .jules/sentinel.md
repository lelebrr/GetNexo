...
## 2026-02-10 - Insecure Default Masquerading as Security Check
**Vulnerability:** `chat-api/server.js` included a check to exit if `JWT_SECRET` was missing, but it was bypassed by a hardcoded default value assigned immediately before the check.
**Learning:** Default values for security-critical configuration can silently defeat security checks intended to enforce their presence.
**Prevention:** Remove default values for secrets entirely. Use strict validation logic that checks the environment variable directly.
