...
## 2024-05-23 - Test Dependence on Insecure Defaults
**Vulnerability:** Tests relied on the application's insecure default JWT secret to run. Removing the production vulnerability caused tests to crash.
**Learning:** Security hardening often breaks tests that implicitly rely on "developer mode" shortcuts. This coupling discourages security fixes.
**Prevention:** Decouple test configuration from production fallbacks. Explicitly inject test-specific secrets in the test environment setup (e.g., Jest `setupFiles`) so production code can enforce strict security without breaking CI.
