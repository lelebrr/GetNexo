...
## 2025-02-18 - Ineffective Environment Validation
**Vulnerability:** The `JWT_SECRET` validation check was rendered ineffective because the variable was assigned a default fallback value (`process.env.JWT_SECRET || 'secret'`) *before* the check (`if (!JWT_SECRET)`).
**Learning:** Assigning default values to security-critical variables masks missing configuration. Even if a validation check exists later, the default value ensures the check always passes.
**Prevention:** Never assign default values to critical secrets in code. Check `process.env.VAR` directly or assign the env var directly, then validate. Use strict checks (e.g., `if (!process.env.JWT_SECRET)`).
