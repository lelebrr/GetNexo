...
## 2024-06-18 - SQL Injection in ORDER BY clauses
**Vulnerability:** Models `UserSession`, `BehavioralTag`, `BehaviorRule`, and `UserEvent` were interpolating unvalidated `orderBy` and `orderDir` user inputs directly into the SQL query string using `better-sqlite3`. Standard query parameters (`?`) cannot parameterize structural elements like `ORDER BY`.
**Learning:** `better-sqlite3` parameterization only protects values, not column names or directions. Interpolating variables directly into `ORDER BY` creates a critical SQL Injection risk.
**Prevention:** Strictly validate `orderBy` using a strict alphanumeric regex allowlist (e.g. `/^[a-zA-Z0-9_]+$/`) and `orderDir` using an exact match array (e.g. `['ASC', 'DESC']`). Ensure `orderDir` validation safely handles undefined or non-string types.
