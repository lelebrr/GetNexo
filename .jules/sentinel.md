...
**Vulnerability:** A Stored XSS vulnerability was identified in the `/tickets/:id/export` endpoint in `chat-api/routes/support.js` where user-controlled ticket details (title, description, messages) were directly interpolated into an HTML string for export without escaping.
**Learning:** Functions that generate raw HTML by concatenating strings must assume all inputs, particularly database fields sourced from users, are malicious. If an admin opens an exported ticket containing a `<script>` payload, it could execute in the context of the domain the file is hosted on (or local context if downloaded).
**Prevention:** Always use a templating engine that auto-escapes output (like EJS, Handlebars) or implement a robust `escapeHtml` utility function when manually constructing HTML strings from dynamic data.
