...
## 2024-05-22 - Publicly Exposed Sensitive Routes & Frontend Fetch Without Auth
**Vulnerability:** Critical business endpoints (`/api/crm/customers`, `/api/tickets`, etc.) were explicitly whitelisted as public routes in the backend. The frontend relied on this insecurity, making `fetch` calls without authentication tokens.
**Learning:** Security by obscurity or "demo mode" shortcuts often persist into production code. Frontend architectures (like Alpine.js) that lack a centralized HTTP client make it easy for developers to accidentally omit authentication headers.
**Prevention:** Remove sensitive routes from public whitelists immediately. Implement a global `fetch` interceptor on the frontend to automatically attach authentication tokens to all requests, ensuring consistent security posture across the application.
