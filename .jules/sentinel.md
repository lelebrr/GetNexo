...
## 2025-03-01 - [CRITICAL] SQL Injection via Dynamically Assembled UPDATE Statement Keys
**Vulnerability:** Found a critical SQL injection vulnerability in `chat-api/api/tickets.js` (PUT `/api/tickets/:id`). The endpoint dynamically constructed the `UPDATE` statement by directly interpolating JSON request payload keys into the `SET` clause (`${key} = ?`). This allowed an attacker to inject arbitrary subqueries or SQL commands by manipulating the object keys.
**Learning:** Even when using parameterized queries (`?` for values), dynamically building the query structure (like column names in `SET` clauses or `WHERE` conditions) directly from user input (e.g., `Object.keys(req.body)`) bypasses parameterization and introduces severe SQL injection risks.
**Prevention:** Always use an explicit allowlist of expected and valid column names when dynamically building SQL queries. Never trust keys or structure coming from the client payload. If a key is not in the explicit allowlist, it must be ignored or rejected.
