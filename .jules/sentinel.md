...
## 2026-03-26 - SQL Injection in Ticket Updates
**Vulnerability:** The `PUT /api/tickets/:id` endpoint dynamically built an SQL `UPDATE` query using unsanitized keys from `req.body`, allowing potential SQL injection or mass assignment.
**Learning:** Constructing SQL queries dynamically from user input without an explicit allowlist is dangerous. Even if values are parameterized, attacker-controlled column names can alter the query structure.
**Prevention:** Always use an explicit allowlist to validate keys from request payloads before interpolating them into SQL `SET` or `WHERE` clauses.
