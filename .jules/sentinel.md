...
## 2025-03-03 - Stored XSS in Support Ticket HTML Export
**Vulnerability:** The `/tickets/:id/export` endpoint in `chat-api/routes/support.js` concatenated user-controlled inputs (ticket title, description, messages) directly into an HTML string without escaping, leading to a Stored Cross-Site Scripting (XSS) vulnerability.
**Learning:** Export features that generate HTML are prone to XSS if inputs aren't properly sanitized. Attackers can inject payloads that execute when a victim (e.g., an admin) downloads and views the exported file.
**Prevention:** Always use a utility function to escape HTML characters (e.g., `<`, `>`, `&`, `"`, `'`) for all user-controlled data before interpolating it into HTML templates.
