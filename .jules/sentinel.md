...
## 2024-05-23 - Ineffective Security Checks due to Fallbacks
**Vulnerability:** Critical security check (`if (!JWT_SECRET)`) was bypassed because a hardcoded fallback string was assigned to the variable upstream.
**Learning:** Defensive coding (defaults) can sometimes undermine security requirements. If a secret is mandatory, the code must not provide a convenient default that allows it to run insecurely.
**Prevention:** Eliminate fallbacks for secrets. Force the application to fail startup if critical environment variables are missing.
