...
**Vulnerability:** The `/api/support/tickets/:id/export` route manually generated HTML by concatenating unescaped user inputs (like ticket titles, descriptions, and messages), leading to Stored XSS vulnerabilities.
**Learning:** Generating HTML programmatically without using a template engine requires explicit sanitization. Any user-provided data interpolated directly into HTML strings poses a Stored XSS risk, especially in admin-facing export tools.
**Prevention:** Implement and use an `escapeHtml` utility function to safely encode special characters (`&`, `<`, `>`, `"`, `'`) before interpolating dynamic user input into raw HTML templates.
