...
## 2025-05-22 - Unauthenticated Access to CRM and Ticket APIs
**Vulnerability:** The `/api/crm/customers` and `/api/tickets` endpoints were whitelisted as public routes for "demo" purposes, allowing unauthenticated users to list, modify, and delete customer data and support tickets (PII/Write Access).
**Learning:** Whitelisting broad categories of endpoints for convenience (like "demo mode") creates critical security gaps. "Demo" features should rely on authenticated demo accounts, not on bypassing authentication.
**Prevention:** Audit public route whitelists rigorously. Never expose endpoints that return PII or allow state modification (POST/PUT/DELETE) without authentication.
