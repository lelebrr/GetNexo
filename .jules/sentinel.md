...
## 2024-05-23 - Stored XSS in HTML Export Generation
**Vulnerability:** Found a Stored Cross-Site Scripting (XSS) vulnerability in the HTML ticket export route (`/tickets/:id/export`). User-controlled data (ticket title, description, messages, client ID) was being directly interpolated into an HTML string literal without escaping.
**Learning:** Manual HTML string concatenation for exports or reports easily overlooks the necessity of output encoding, leading to dangerous XSS injection points when administrators or users view the generated files.
**Prevention:** Always use an HTML escaping utility (e.g., `escapeHtml`) to sanitize any dynamic data or user-provided input before interpolating it into HTML templates or string literals.
