...
## 2026-02-21 - Sensitive Routes Explicitly Whitelisted
**Vulnerability:** Critical endpoints like `/api/crm/customers`, `/api/tickets`, and `/api/settings` were explicitly added to the `publicRoutes` whitelist in `server.js`, bypassing global authentication.
**Learning:** Hardcoded whitelists for "demo/initial setup" often get left in production code. Comments like "allow without strict auth" are red flags.
**Prevention:** Remove "development mode" bypasses before merging to main. Use feature flags or environment variables if auth bypass is needed for local dev, but never default to insecure.
